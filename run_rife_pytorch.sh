...
# If INFILE is a directory (frames were passed), copy or symlink frames into TMP_DIR/input
# and skip the ffmpeg extraction step later.
EXTRACT_SKIPPED=0
if [ -d "$INFILE" ]; then
  log "INFILE is a directory: $INFILE. Treating as frames directory. Preparing TMP input..."
  # Prefer PNG/JPG files
  shopt -s nullglob
  files=($INFILE/*.{png,png,jpg,jpeg})
  shopt -u nullglob
  if [ ${#files[@]} -gt 0 ]; then
    for f in "${files[@]}"; do
      # use symlinks to save space if possible
      ln -sf "$f" "$TMP_DIR/input/$(basename "$f")" 2>/dev/null || cp -f "$f" "$TMP_DIR/input/" 2>/dev/null || true
    done
    EXTRACT_SKIPPED=1
    log "Copied/symlinked ${#files[@]} frames into $TMP_DIR/input"
  else
    log "INFILE directory provided but contains no image files: $INFILE"
    # leave EXTRACT_SKIPPED=0 to attempt ffmpeg extraction (which will likely fail)
  fi
fi
...
if [ "$EXTRACT_SKIPPED" -eq 1 ]; then
  log "Skipping ffmpeg extraction since input was provided as frames directory"
else
